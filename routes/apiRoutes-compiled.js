...
apiRoutes.get('/profile/:username', function (req, res) {
  ...
  User.findOne({ username: req.params.username }, '_id username followedBy followedByCount profilePicture coverPicture').exec((err, user) => {
    if (err) {
      res.status(400).json({
        success: 'false',
        error: err
    ...
    if (user) {
      Music.find({ userId: user.id }).sort({ 'createdAt': -1 }).limit(1000).exec((err, musics) => {
        if (user.followedBy.indexOf(req.decoded.user.id) > -1) followed = true;
...
apiRoutes.post('/follow', (req, res) => {
  ...
  User.findOne(req.decoded.user.id, 'followingCount following').exec((err, currentUser) => {
    if (currentUser) {
      User.findOne({ username: toFollow }, 'followedByCount followedBy').exec((err, user) => {
        if (user) {
...
apiRoutes.get('/musicSearch', (req, res) => {
  ...
  User.findById(req.decoded.user.id, 'following username').exec(function (err, user) {

    console.log(user);
    Music.find({
      $and: [{ userId: { $in: user.following } }, { name: { $regex: pattern, $options: "i" } }]
    }).sort({ createdAt: -1 }).limit(30).exec((err, musics) => {
      res.send(musics);
    });
  });
});
...
apiRoutes.get('/discover', (req, res) => {
  User.findById(req.decoded.user.id, 'following username').exec(function (err, user) {
    Music.find({ userId: { $in: user.following } }).sort({ createdAt: -1 }).limit(200).exec((err, musics) => {
      res.send(musics);
    });
  });
});
...
apiRoutes.get('/userSearch', (req, res) => {
  User.find({ username: new RegExp(req.query.search, "i") }, 'username').limit(5).exec((err, users) => {
    res.send(users);
  });
});
...
apiRoutes.get('/genreMusics', (req, res) => {
  ...
  User.findById(req.decoded.user.id, 'following username').exec(function (err, user) {
    Music.find({
      $and: [{ userId: { $in: user.following } }, { genres: genre }]
    }).sort({ createdAt: -1 }).limit(200).exec((err, musics) => {
      res.send(musics);
    });
  });
});
...
apiRoutes.put('/updatePicture', (req, res) => {
  ...
  User.findById(id, function (err, user) {
    ...
    if (text === 'profile') user.profilePicture = filename;
    if (text === 'cover') user.coverPicture = filename;
...
//# sourceMappingURL=apiRoutes-compiled.js.map